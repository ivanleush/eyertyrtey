...
def register_handlers(bot_instance):
    ...
    @bot.callback_query_handler(func=lambda call: True) #  Добавляем декоратор здесь
    def callback_inline_wrapper(call): # Оборачиваем функцию
        return callback_inline(call)